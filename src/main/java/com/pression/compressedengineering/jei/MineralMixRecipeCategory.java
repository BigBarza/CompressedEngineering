...
public class MineralMixRecipeCategory implements IRecipeCategory<MineralMix> {
    ...
    public MineralMixRecipeCategory(IGuiHelper guiHelper){
        ...
        this.icon = guiHelper.createDrawableIngredient(VanillaTypes.ITEM_STACK, new ItemStack(IEBlocks.Multiblocks.EXCAVATOR));
    ...
    @Override
    public void draw(MineralMix recipe, IRecipeSlotsView slotsView, PoseStack ms, double mouseX, double mouseY) {
        ...
        font.draw(ms, veinNameText, ((float) background.getWidth() / 2)-((float) font.width(veinNameText.getString()) /2), 0, 0x888888);
        font.draw(ms, failChanceText, ((float) background.getWidth() / 2)-((float) font.width(failChanceText.getString()) /2), 52, 0x888888);
        ...
        for(int i = 0; i<recipe.outputs.length; i++){
            ...
            drawLine(ms, xOffset, 28, xOffset, 32, 0xFF666666);
            GuiComponent.fill(ms, xOffset-1, 31, xOffset+1,33, 0xFF666666);
            drawLine(ms, xOffset, 32, x2, 36, 0xFF666666);
            GuiComponent.fill(ms, x2-1, 35, x2+1, 37, 0xFF666666);
            drawLine(ms, x2, 36, x2, 40, 0xFF666666);

            GuiComponent.fill(ms, (int) barFilled, 40, (int) (barFilled+output.chance()*150), 47, colours.get(col));
            col++;
        ...
        for(int i = 0; i<recipe.spoils.length; i++){
            ...
            xOffset += (int) (area*output.chance()+8);
            int x2 = Math.round(barFilled+(output.chance()*75));
            drawLine(ms, xOffset, 80, xOffset, 84, 0xFF666666);
            GuiComponent.fill(ms, xOffset-1, 83, xOffset+1,85, 0xFF666666);
            drawLine(ms, xOffset, 84, x2, 88, 0xFF666666);
            GuiComponent.fill(ms, x2-1, 87, x2+1, 89, 0xFF666666);
            drawLine(ms, x2, 88, x2, 92, 0xFF666666);

            GuiComponent.fill(ms, (int) barFilled, 92, (int) (barFilled+output.chance()*150), 99, colours.get(col));
            col++;
            if(col >= colours.size()) col = 0;
            barFilled += output.chance()*150;
            xOffset += (int) (area*output.chance()+8);
